...
def score_product(product: Dict, profile: Dict) -> int:
    ...
    if category == "life" and profile.get("has_dependants"):
        score += 40
    elif category == "accident" and profile.get("employment_status") in ["medium", "high"]:
        score += 35
    ...
    # BUDGET FIT (25)
    # -------------------------
    budget = float(profile.get("monthly_income") or 0)
    if budget > 0:
        if premium <= budget:
            score += 25
...
def why_this_matters(
    ...
    if category == "accident":
        if profile.get("employment_status") in ["medium", "high"]:
    ...
    if premium <= float(profile.get("monthly_income") or 0):
